        ...
        <>
          ...
          {order.paymentMethod === 'COD' && (
            <div className={`alert mt-4 ${order.codDiscrepancyStatus === 'Open' ? 'alert-warning' : 'alert-secondary'}`}>
              <strong>COD dự kiến:</strong> {formatCurrency(order.codExpectedAmount)}
              {' · '}<strong>Đã thu từ khách:</strong> {formatCurrency(order.customerCollectedAmount)}
              {' · '}<strong>Carrier đã đối soát:</strong> {formatCurrency(order.carrierSettlementAmount)}
              <div>Chênh lệch COD: {order.codDiscrepancyStatus}; trạng thái đối soát Carrier: {order.settlementReconciliationStatus}</div>
              {order.codRecoveryReceiptId && <div>Phiếu thu hồi COD: {order.codRecoveryReceiptId}</div>}
            </div>
          )}
