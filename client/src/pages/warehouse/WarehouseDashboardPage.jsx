...
export default function WarehouseDashboardPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [inventory, lowStock, exports] = await Promise.all([
          inventoryService.listInventory().catch(() => []),
          inventoryService.listLowStock().catch(() => []),
          inventoryService.listStockExports().catch(() => []),
        ]);
        if (!cancelled) {
          const invArr = Array.isArray(inventory) ? inventory : [];
          const lowArr = Array.isArray(lowStock) ? lowStock : [];
          const expArr = Array.isArray(exports) ? exports : [];
          setStats({
            totalItems: invArr.length,
            lowStock: lowArr.length,
            pendingExports: expArr.filter((e) => e.status === 'Pending').length,
          });
        }
      } catch (err) {
        if (!cancelled) {
