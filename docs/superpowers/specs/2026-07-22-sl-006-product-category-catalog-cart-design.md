...
|---|---|---|---|---|---|---|---|---|---|
| SL-006 | passed | passed | passed | ready | not-started | not-started | not-started | not-started | Complete exact G3 API/interface/code/test/release-evidence mapping against the reconciled SRS revision |
...
|---|---|---|---|---|---|---|
| SRC-035 | [Google SRS](https://docs.google.com/document/d/1j_1Qg_DoFC6Dk5zk_UZcnMnjjqW2wjKPNAH1ZNxNwtE/edit?tab=t.0) | Google Docs revision `AIroW372r8j-BncuGRhIEqFwCQa2PLXsnMT53H_cxn5r7E_t-NkRjh2gJg2UEves9dhsAFtoTr0qoSWM8Lt1qYAOQzSFBEWVaj2Ap2eXHQI`; one tab `t.0`; read back 2026-07-23 | Candidate catalog/cart/public best-seller text plus the bounded public-versus-historical report distinction | Candidate source except where approved; CR-001 v2.1 is normative for the boundary it states | SRS contributors; Project Business Approver approves policy | Complete SL-006 publication/cart G3 mapping remains; the CR-001 addendum does not replace the package lifecycle |
...
| SRC-055 | [`CR-001 v2.1`](2026-07-23-cr-001-cross-slice-business-closure-v2.md) | Approved 2026-07-23 | Binding distinction between current public best sellers and historical Admin CompletedSale/Refund reporting, using Customer collection rather than Carrier settlement time | Normative cross-slice reconciliation | Project Business Approver | Corrects conflicting candidate SRS wording without creating a new ranking policy |
...
| BD-070 | SL-006 | What qualifies and ranks a best-selling Product? | Newest Products; gross Orders regardless of outcome; net completed sales window | Rank up to ten currently public Products using quantities from Orders whose immutable `CompletedSaleAt` falls in the current Vietnam-time thirty-calendar-day window and whose current Order/Payment facts are `Delivered + Paid`, excluding Orders whose whole Return/Refund completed as `Returned`. Exchange creates no new sale. Ties use Order-detail snapshot revenue then SKU. With no qualifying sale, show newest Products under the distinct label “Sản phẩm mới” | Avoid calling cancelled, unpaid, returned, exchanged, or merely recent Products best sellers and give later Customer COD collection one unambiguous window timestamp independent of Carrier remittance | Project Business Approver | 2026-07-22 | BR-066 |
...
| BR-066 | BestSeller calculation at request time shall use the interval from 00:00:00 on the Vietnam calendar date twenty-nine days before the request through the request instant in `Asia/Ho_Chi_Minh`. It shall sum OrderDetail quantity only when immutable `CompletedSaleAt` falls in that interval and the Order is currently `Delivered` with Payment `Paid`; `CompletedSaleAt` follows CR `BR-108/BR-121`, so later full Customer COD collection uses actual later `PaidAt`, while Carrier remittance/settlement time never moves the sale window. It shall exclude a whole Order once `SL-001` completes it as `Returned` and create no additional sale from `SL-002` Exchange or replacement fulfillment. Only currently public Products qualify. Rank descending by qualifying quantity, then qualifying OrderDetail snapshot revenue, then canonical SKU ascending, and return at most ten. If none qualifies, return newest currently public Products under “Sản phẩm mới”, never under “Bán chạy”. | BD-070, BD-111, BD-117 |
...
6. This public current projection is not the historical Admin Gross Sales/Product report. A later completed whole Return excludes the Order from current public ranking but leaves the immutable historical CompletedSale and separate Refund under `SL-009`.
...
17. `BestSellerQuantity = sum(OrderDetail.Quantity)` and `BestSellerRevenue = sum(OrderDetail.Subtotal)` only for qualifying current Delivered/Paid/non-Returned Orders whose `CompletedSaleAt` falls inside the approved window; each Subtotal is the immutable line snapshot approved in `SL-003`, and current Product price is never used for historic revenue.
...
| AT-113 | Given online/normal-COD/later-Customer-collected-COD Orders and a full-customer-collection/late-Carrier-settlement Order around the exact Vietnam-time `CompletedSaleAt` boundary, when best sellers calculate, then Customer collection time—not Carrier settlement time—controls the qualifying window and ranking uses quantity, snapshot revenue, then SKU for at most ten public Products. | `approved-requirement` |
...
| BD-070, BD-106, BD-107 | CR BR-119 | Public best-seller query versus Admin historical report | Home/public Product projection; report service/Admin dashboard | CR AT-218, AT-219 | Candidate SRS conflates gross completed-sale events with the public current ranking | ready |
...
4a. The same completed Return does not erase the historical CompletedSale from `SL-009`; Admin Gross and Refund remain separate immutable event measures.
...
No implementation plan, migration, or code change is authorized by this document alone. CR-001 v2.1 records the completed cross-system closure and COD collection/settlement clarification; the next step is exact G3 mapping before red acceptance tests or implementation.
