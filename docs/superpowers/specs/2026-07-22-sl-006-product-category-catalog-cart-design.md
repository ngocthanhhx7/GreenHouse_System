...
| SRC-055 | [`CR-001 v2`](2026-07-23-cr-001-cross-slice-business-closure-v2.md) | Approved 2026-07-23 | Binding distinction between current public best sellers and historical Admin CompletedSale/Refund reporting | Normative cross-slice reconciliation | Project Business Approver | Corrects conflicting candidate SRS wording without creating a new ranking policy |
...
| BR-066 | BestSeller calculation at request time shall use the interval from 00:00:00 on the Vietnam calendar date twenty-nine days before the request through the request instant in `Asia/Ho_Chi_Minh`. It shall sum OrderDetail quantity only when immutable `CompletedSaleAt` falls in that interval and the Order is currently `Delivered` with Payment `Paid`; `CompletedSaleAt` follows CR `BR-108`, so a later COD collection uses actual later `PaidAt`. It shall exclude a whole Order once `SL-001` completes it as `Returned` and create no additional sale from `SL-002` Exchange or replacement fulfillment. Only currently public Products qualify. Rank descending by qualifying quantity, then qualifying OrderDetail snapshot revenue, then canonical SKU ascending, and return at most ten. If none qualifies, return newest currently public Products under “Sản phẩm mới”, never under “Bán chạy”. | BD-070, BD-111 |
...
| AT-113 | Given online/normal-COD/later-collected-COD Orders around the exact Vietnam-time `CompletedSaleAt` boundary with current Delivered/Paid and disqualifying statuses, when best sellers calculate, then only qualifying OrderDetail snapshots contribute and ranking uses quantity, snapshot revenue, then SKU for at most ten public Products. | `approved-requirement` |
