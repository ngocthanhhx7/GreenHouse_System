...
|---|---|---|---|---|---|---|
| SRC-025 | [Google SRS](https://docs.google.com/document/d/1j_1Qg_DoFC6Dk5zk_UZcnMnjjqW2wjKPNAH1ZNxNwtE/edit?tab=t.0) | Google Docs revision `AIroW372r8j-BncuGRhIEqFwCQa2PLXsnMT53H_cxn5r7E_t-NkRjh2gJg2UEves9dhsAFtoTr0qoSWM8Lt1qYAOQzSFBEWVaj2Ap2eXHQI`; one tab `t.0`; read back 2026-07-23 | Candidate fulfillment/delivery text plus the bounded COD discrepancy and delivery-failure closure | Candidate source except where approved; CR-001 v2.1 is normative for its bounded COD and notification rules | SRS contributors; Project Business Approver approves policy | Legacy delivery text remains candidate; the CR-001 v2.1 addendum supersedes its COD/reconciliation conflict |
...
| SRC-055 | [`CR-001 v2.1`](2026-07-23-cr-001-cross-slice-business-closure-v2.md) | Approved 2026-07-23 | CODDiscrepancy resolution, separate Customer collection/Carrier settlement facts, held after-sales rights, evidence-based CompletedSale clock, independent money obligations, and missing notification handoffs | Normative cross-slice authority | Project Business Approver | Closes the unsafe intersection between `Delivered + Unpaid` and full Return Refund eligibility |
...
| BD-045 | SL-004 | How is COD completed or represented when collection fails? | Manual Staff Paid button; always couple delivery/Paid; normal atomic path plus discrepancy | Normal full Customer collection commits `Delivered + Paid` together using fixed server-derived `CODExpectedAmount` and Carrier collection evidence. Staff cannot choose/enter a payment amount. Proven physical delivery without confirmed full Customer collection becomes `Delivered + Unpaid` with an open `CODDiscrepancy`, never false Paid. Carrier remittance/settlement is recorded separately and cannot downgrade a valid Paid fact | Preserve physical delivery, Customer money, and Carrier settlement as separate facts under abnormal behavior | Project Business Approver | 2026-07-22 | BR-041, CR BR-121 |
...
| Carrier | Transport complete parcels, attempt delivery, collect the fixed COD amount when applicable, and provide separate collection/settlement evidence | External handoff/delivery/attempt/return/loss/damage/collection/remittance facts through integration or evidence | Log in as a GreenHouse role; decide eligibility/refund; edit Order/Inventory/payment amount | External Shipment and money-evidence facts only; GreenHouse validates before internal transition | Minimum receiver/shipment data, fixed expected COD amount, Customer collection evidence, and settlement/remittance evidence | Evidence to System/Staff; returned parcel to Warehouse; settlement discrepancy to Staff reconciliation | Unknown/disputed evidence remains non-terminal or opens the correct reconciliation; remittance delay is not Customer under-collection |
...
| BR-041 | For normal COD delivery, `CODExpectedAmount` is fixed at the server-derived `Order.TotalAmount`; verified full `CustomerCollectedAmount` and verified physical delivery shall atomically set Order to Delivered and Payment to Paid; no actor enters or chooses the amount. If physical delivery is proven without verified full Customer collection, Order becomes Delivered, Payment remains Unpaid, and exactly one open `CODDiscrepancy` is created for reconciliation. `CarrierSettlementAmount` is a separate remittance fact: a late/partial settlement does not downgrade a valid Paid fact or create a Refund. No Staff-only click may establish Paid. A timely after-sales request may be recorded under CR BR-106, but no Return Refund/destination/payout path opens before full Customer collection is verified. | BD-045, BD-110, BD-117 |
...
7. For normal COD, the same verified outcome proves full `CustomerCollectedAmount = CODExpectedAmount`; System changes Shipped to Delivered and Payment Unpaid to Paid atomically.
...
| AF-004-16 | Physical COD delivery is proven but full `CustomerCollectedAmount` is not | Set Delivered/DeliveredAt and five-day deadlines, leave Payment Unpaid, create one CODDiscrepancy, and never assert Paid. A Carrier remittance delay after full Customer collection follows separate settlement reconciliation instead |
...
|---|---|---|---|
| Open | Full Customer collection proved at delivery | `CustomerCollectedAmount = CODExpectedAmount`; set `PaidAt=DeliveredAt`; release eligible after-sales hold; track any Carrier settlement separately | ResolvedCollectedAtDelivery |
| Open | Full Customer collection proved later | `CustomerCollectedAmount = CODExpectedAmount`; set `PaidAt` to actual Customer-collection instant; release eligible after-sales hold; track any Carrier settlement separately | ResolvedCollectedLater |
| Open | `CustomerCollectedAmount < CODExpectedAmount` is conclusively proved | Derive the actual Customer-collected balance from attributable evidence; store Carrier settlement separately; block normal Return/Exchange; begin complete goods recovery | RecoveryRequired |
| Open | Full Customer collection is proved but Carrier settlement is late, partial, or disputed | Set/retain Paid and evidenced PaidAt; open a separate settlement-reconciliation work item; create no Refund and no second COD portion | ResolvedCollectedAtDelivery or ResolvedCollectedLater + settlement reconciliation open |
| RecoveryRequired | Warehouse accounts all goods and CustomerCollectedAmount is zero | Post each authorized Inventory movement once; set Order `Returned`, Payment `Cancelled`; close held case with no Refund | ResolvedUncollected |
| RecoveryRequired | Warehouse accounts all goods and CustomerCollectedAmount is positive but below CODExpectedAmount | Create one exact server-derived COD-recovery Refund equal to CustomerCollectedAmount through the secure destination flow; retain not-Paid primary state | RecoveryRefundPending |
...
8. Payment state expresses verified Customer money independently from physical delivery and Carrier settlement. Normal COD finishes delivery plus full Customer collection together; proven delivery without full Customer collection is represented explicitly, never hidden as Shipped or Paid. A settlement mismatch cannot downgrade Paid or create a Refund.
9. `CODExpectedAmount = Order.TotalAmount` is fixed, and the current release has no split/installment COD. Customer collection and Carrier settlement amounts are evidence-backed, stored separately, and never actor-entered as discretionary values; approved Refund amounts remain server-derived.
...
- Delivery/COD screens show fixed `CODExpectedAmount`, verified/masked Customer-collection status, and separate Carrier-settlement reconciliation status; they accept evidence/status actions, not an editable payment amount or split-COD control.
...
| AT-065 | Given COD with verified physical delivery and verified `CustomerCollectedAmount = CODExpectedAmount`, when processed, then Delivered and Paid plus timestamps/deadlines commit atomically using server-derived total; no editable amount, split-COD control, or separate Staff Paid action exists. | `approved-requirement` |
| AT-066 | Given physical COD delivery is proven but full Customer collection is not, when recorded, then Order is Delivered, Payment remains Unpaid, exactly one CODDiscrepancy opens, and no false Paid evidence exists. Given full Customer collection with late/partial Carrier settlement instead, Payment remains Paid and only separate settlement reconciliation opens. | `approved-requirement` |
...
| BD-110, BD-111, BD-114, BD-117 | CR BR-106 through BR-108, BR-113 through BR-115, BR-121 | COD hold/reconciliation; Customer collection versus Carrier settlement; CompletedSale; independent obligations | CODDiscrepancy, Carrier settlement work item, Return/Exchange guard, PaymentAttempt/Refund, report events | CR AT-205 through AT-208, AT-215 through AT-218, AT-223 through AT-226 | Current COD action and refund model cannot preserve these independent facts | ready |
...
1. Physical `Delivered` always starts immutable after-sales deadlines, including when Customer COD collection is disputed.
...
3. CompletedSale timing follows verified Customer collection evidence: full Customer collection at delivery uses `DeliveredAt`; actual later Customer collection uses later `PaidAt`; Carrier settlement time never controls CompletedSale and a settlement delay never creates a Refund.
...
5. Conclusively verified Customer under-collection uses CR `BR-107`: complete goods recovery is mandatory; zero Customer collection closes with no Refund, while a positive `CustomerCollectedAmount` creates one exact server-derived COD-recovery Refund and closes only after verified payout. Partial Carrier remittance is not this branch.
