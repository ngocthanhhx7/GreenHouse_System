...
| SRC-055 | [`CR-001 v2`](2026-07-23-cr-001-cross-slice-business-closure-v2.md) | Approved 2026-07-23 | Later COD CompletedSale clock, distinct Refund obligations, aggregate money settlement, public/report separation, destination/evidence privacy, and missing fulfillment notifications | Normative cross-slice authority | Project Business Approver | Refines BR-096, BR-102, BR-103, notification matrix, and report boundaries |
...
| BR-102 | A CompletedSale event shall be established only when an Order has immutable `DeliveredAt` and verified successful full collection. Collection at/before delivery contributes immutable `Order.TotalAmount` to Gross Sales at `DeliveredAt`; COD collection that actually occurs later contributes at the actual later `PaidAt`. Later Return, cancellation from another technical state, or Refund does not erase that event. Refunds equal amounts of distinct Refund obligations becoming `Refunded` in the selected period at `RefundedAt`, including cancellation, verified late/excess payment, failed delivery, COD recovery, and Return triggers. `NetSales = GrossSales - Refunds` may be negative. Pending, unverified/unpaid, Failed, Cancelled-payment, or never-validly-delivered Orders contribute no Gross Sales. | BD-106, BD-111, BD-114 |
...
- `CompletedSaleAt = DeliveredAt` when verified full collection occurred at or before delivery; when COD full collection actually occurs later, `CompletedSaleAt = PaidAt` using that later collection instant.
...
| BD-111, BD-114 | CR BR-108, BR-113 through BR-115, BR-119 | CompletedSale/Refund events and aggregate settlement | Report/event projections; Refund and PaymentAttempt sources | CR AT-207, AT-208, AT-215 through AT-218 | Current report and refund storage infer from mutable status and one Order-level refund row | ready |
...
- Google SRS full content and revision/tab metadata were read before the cross-slice reconciliation; the CR-001 v2 addendum was then written with a revision guard and read back.
...
- The approval artifact changes no application code, migration, runtime data, or user-owned `docs/presentation/` content; the bounded Google SRS sync is recorded by CR-001 v2.
