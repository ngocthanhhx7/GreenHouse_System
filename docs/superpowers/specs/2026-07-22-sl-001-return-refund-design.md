...
|---|---|---|---|---|---|---|---|---|---|
| SL-001 | passed | passed | passed | ready | not-started | not-started | not-started | not-started | Reconcile every approved rule to SRS, interface, code, test, and release evidence |
...
|---|---|---|---|---|---|---|
| SRC-001 | [Google SRS](https://docs.google.com/document/d/1j_1Qg_DoFC6Dk5zk_UZcnMnjjqW2wjKPNAH1ZNxNwtE/edit?tab=t.0) | Revisions in the document header | Candidate UC-CS-12, FR-RR, BR-RR, state, and acceptance text | Candidate source only where an approved decision adopts it | SRS contributors; Project Business Approver approves policy | Still labeled `Text Baseline Candidate` and does not contain the complete approved destination, deadline, and recovery lifecycle |
...
| SRC-024 | Approved `SL-002 Exchange Design` and its explicit approval source SRC-012 | Commit `fbe9c57`; 2026-07-22 | BD-021 one-active-after-sales-case boundary shared with Return/Refund | Normative cross-slice authority for BR-RR-13 | Project Business Approver | Google SRS does not yet contain the shared guard |
...
|---|---|---|---|---|---|---|---|---|
| BD-001 | SL-001 | Which lifecycle is normative? | Atomic whole-Order lifecycle; current split lifecycle | Whole-Order return with atomic Warehouse receipt, Inventory movements, one Refund, and payment handoff | Prevent partial receipt, lost stock, duplicate Refund, and inconsistent state | Project Business Approver | 2026-07-22 | BR-RR-01, BR-RR-02, BR-RR-07 through BR-RR-14 |
...
| BR-RR-02 | Return scope is the complete Order. Backend derives `RefundAmount = Order.TotalAmount`; no actor may choose or alter it. | BD-001, BD-005 |
...
| BR-RR-08 | One atomic Warehouse outcome creates sellable and damaged Inventory transactions, changes the request to `Received`, creates exactly one Refund, and sets Order payment to `RefundPending`. Any failure commits none of those effects. | BD-001 |
...
| BR-RR-14 | Completion requires goods `Received`, Refund `Refunded`, Order payment `Refunded`, request `Completed`, and Order `Returned` exactly once. | BD-001 |
...
14. Verified automatic or manual success sets Refund and Order payment to `Refunded` once.
...
Return Request business states are `New`, `Approved`, `Rejected`, `Expired`, `Received`, and `Completed`. Destination and Refund have independent lifecycles; they must not be compressed into Return Request state.
...
| AT-011 | Given every purchased line exactly once with valid quantities, when Warehouse confirms, then one atomic commit records Inventory, `Received`, one Refund, and `RefundPending`. | `approved-requirement` |
...
## 12. Method Basis and Next Phase
...
No implementation plan or code change is authorized by this document alone. The project is intentionally continuing through the remaining business packages before the final cross-system SRS baseline and implementation roadmap.
