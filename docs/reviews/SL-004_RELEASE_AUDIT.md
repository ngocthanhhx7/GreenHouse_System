...
| Demo escape hatch could leak into production | Runtime guard rejects Staff `codCollectionResult` in production; Carrier signature remains required, and the UI consumes a server-projected capability instead of client build mode. |
...
Supplemental verification result: focused server `39/39`, focused client
`16/16`, full server `1064/1064`, full client `262/262`, production build PASS
