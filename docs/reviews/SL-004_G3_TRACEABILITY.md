...
| Legacy Delivered records remain awaiting confirmation | `migrateCustomerDeliveryReceipt.js` creates only receipt indexes and verifies Shipment guard compatibility; it writes no receipt row | Initial migration RED 0/6 then GREEN 6/6; command-identity P1 RED 6/7 then GREEN 7/7; dry-run and repeat-apply contracts assert zero business writes |
| Exact technical indexes cannot silently drift | Migration preflight rejects duplicate Customer/idempotency command identities with `CUSTOMER_DELIVERY_RECEIPT_COMMAND_AMBIGUOUS`, duplicate terminal/initial receipt identities, unsafe guard types, and same-key index semantic conflicts before every index operation | `migrateCustomerDeliveryReceipt.test.js` |
