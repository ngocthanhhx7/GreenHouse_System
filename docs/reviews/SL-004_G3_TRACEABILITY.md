...
| Exact technical indexes cannot silently drift | Migration preflight uses bounded server-side group counts, rejects duplicate Customer/idempotency command identities with `CUSTOMER_DELIVERY_RECEIPT_COMMAND_AMBIGUOUS`, duplicate terminal/initial receipt identities, unsafe guard types, and same-key index semantic conflicts before every index operation | `migrateCustomerDeliveryReceipt.test.js`; 5,000-row synthetic history asserts no `$push`, reason read, or unbounded diagnostic |
